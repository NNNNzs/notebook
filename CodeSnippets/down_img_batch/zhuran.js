...
const bakJson = require('./1.json')
const axios = require('axios');
const getPixels = require("get-pixels")
const fs = require('fs')
const savePath = '/Users/nnnnzs/Pictures/whg1'
...
async function init() {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (absolutePath) {
      fileName = fileName.replace('http://www.zrbwg.net.cn','');
    }
    ...
    try {
      ...
      getPixels(path, (err, data) => {
        if (err) {
          ...
          fs.appendFileSync('./error.txt', `error ${path}\n\n`)
        }
      })
    } catch (e) {
